  ...
  Store.prototype = {
    ...
    _organiseData: function(dataPoint, forceRender) {
        ...
        if (!store[x][y]) {
          ...
          //change1:store[x][y] += value;
          //store[x][y] += value;
           store[x][y] =store[x][y] < value?value:store[x][y];
           // store[x][y]= store[x][y] && store[x][y]>0?(store[x][y]+value)/2:value;
  ...
  Canvas2dRenderer.prototype = {
    ...
    _drawAlpha: function(data) {
      ...
      while(dataLen--) {
        ...
        //思路1：效果不对
        //this._templates[radius] = tpl=this._changeImgAlpha(tpl,rectX,rectY,radius*2,radius*2);
        //shadowCtx.drawImage(tpl, rectX, rectY);

         //思路1：取原地图对应区域的透明度对应的值，与当前图片值进行比较
         var orgMaxValue=this._getMaxValue(rectX,rectY,radius*2,radius*2);
         if(value>orgMaxValue) {
      ...
      _changeImgAlpha:function(newImgCanvas,x, y, width, height){
          //思路：将新生成的图片与区域内的原像素点的透明度进行比较，只保留比较大的像素值
          var orgImg = this.shadowCtx.getImageData(x, y, width, height);
          var orgData = orgImg.data;
          var newImg=newImgCanvas.getContext("2d").getImageData(0,0,width,height);
          var newData=newImg.data;
          var len = orgData.length;
          for (var i = 3; i < len; i+= 4) {
              var orgAlpha = orgData[i];
              var newAlpha = newData[i];
              if(orgAlpha>newAlpha)
              {
                  newData[i]=1;
              }
              else {
          ...
          orgImg.data=orgData;
          this.shadowCtx.putImageData(orgImg,x,y);

          newImg.data=newData;
          newImgCanvas.getContext("2d").putImageData(newImg,0,0);
          return newImgCanvas;
      },
      _getMaxValue:function(x,y,width,height){
          var maxAlpha=0;
          ...
          value = (Math.abs(max-min) * (maxAlpha/255)) >> 0;
          return value;
      },
    _colorize: function() {
