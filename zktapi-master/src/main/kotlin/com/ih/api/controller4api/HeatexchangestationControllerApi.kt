...
@CrossOrigin("*")
@RequestMapping("/api/heatexchangestation/")
@Controller
class HeatexchangestationControllerAPI {
    @Resource
...
/*
    ...
    //  bindCompanyID--约束公司ID，不能超过其范围，searchCompanyID--选择的公司ID ,parentID-中心站的ID
    ...
    fun getPaging(@RequestParam(name = "bindCompanyID", defaultValue = "-1", required = false) bindCompanyID: Int,
        ...
        if( bid ==  -1 )
            bid = bindCompanyID
        //查找id=bindCompanyID或searchCompany的公司，当
    ...
    //    { //查找公司及母公司ID为companyID的下属换热站  (com.id = #{companyID} or  com.ParentID = #{companyID})
...
fun getStations(    @RequestParam(name = "companyID", defaultValue = "-1", required = false) companyID: Int,
        ...
        var counts = _service?.getCount("",-1 , companyID)
