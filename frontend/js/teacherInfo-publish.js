 ...
 * 裸路径由 utility_request.js 的 normalizeUrl 补 /api/v1 前缀。
 *
 * 依赖：api.js (escapeHtml/escapeAttr/applyTerms)、teacherInfo.js 的模块状态、全局 html2canvas（CDN）
 */
...
/**
 * 构造「教师公开介绍页」地址。
 * 公开页免登录，**刻意不带 tCode**：它是要发给家长的对外分享链接，
 * 带上租户编码既无用又污染链接；页面本身按 id/teacherId 定位数据。
 */
function publicProfileUrl(params) {
  // 取当前页面所在目录（兼容部署在子路径的情况），再拼公开页文件名
  const base = location.pathname.replace(/teacherInfo\.html.*$/, '');
...
function swapFieldOrder(srcKey, targetKey) {
  ...
  const srcRow = rows.find(r => r.getAttribute('data-field-key') === srcKey);
  if (!srcRow) return;
  const parent = srcRow.parentElement;
  // 仅支持同组（同一 parent）内交换
...
function generatePublishHtml(mode) {
  ...
  const rowsHtml = fields.map(f => {
    ...
    if (f.key === 'certificates') {
      ...
      return `<section style="margin-bottom:16px;">
        <h3 style="margin:0 0 8px 0;color:${escapeAttr(style.accentColor)};font-size:${style.fontSizePx + 2}px;">资格证书</h3>
    ...
    if (f.key === 'bioText') {
      if (!v) return '';
      return `<section style="margin-bottom:16px;">
        <h3 style="margin:0 0 8px 0;color:${escapeAttr(style.accentColor)};font-size:${style.fontSizePx + 2}px;">简介</h3>
  ...
  if (basicKeys.some(k => fset.has(k))) {
    ...
    basicHtml = `<section style="margin-bottom:16px;">
      <h3 style="margin:0 0 8px 0;color:${escapeAttr(style.accentColor)};font-size:${style.fontSizePx + 2}px;">基本信息</h3>
  ...
  // 其余字段已在 rowsHtml 的 key-value 区块处理过，这里只留占位
  ...
  return `<div class="publish-wrapper" style="font-family:${escapeAttr(style.fontFamily)};font-size:${style.fontSizePx}px;color:#222;background:${escapeAttr(style.bgColor)};padding:24px;border-radius:12px;">
    <style>${cssBlock}</style>
    ${bodyInner}
  </div>`;
}
...
function renderPublishPreview(mode) {
  ...
  try {
    ...
    if (typeof applyTerms === 'function') applyTerms(preview);
  } catch (e) {
...
async function enterPublishMode() {
  ...
  const onceToken = '__pubTitleBound';
  if (!titleEl[onceToken]) {
    titleEl.addEventListener('input', schedulePublishPreview);
    titleEl[onceToken] = true;  // 避免反复进出发布模式重复绑定同一个事件
  }
...
async function submitPublish() {
  ...
  try {
    ...
    if (res && res.publishedProfileId) {
      ...
      document.getElementById('pub-history').value = res.publishedProfileId;
      // 按教师ID给出最新发布版本的公开链接
      const link = publicProfileUrl({ teacherId: currentTeacherId });
      alert(`发布成功！\n\n公开访问链接：\n${link}\n\n（已复制到剪贴板）`);
...
/**
 * html2canvas 区域截图工具（保存为图片用）
 * 依赖：全局 html2canvas（CDN 引入）
 */
const sleep = (ms) => new Promise((r) => setTimeout(r, ms));

/**
 * 递归展开 target 内所有可滚动子容器（overflow:auto/scroll 且有隐藏内容），返回恢复函数。
 */
 ...
 * @param {Object} [options] scale / fullContent / backgroundColor
 */
async function captureElement(target, options = {}) {
  const el = typeof target === 'string' ? document.querySelector(target) : target;
  if (!el) throw new Error('找不到目标元素: ' + target);
  if (typeof html2canvas !== 'function') throw new Error('html2canvas 未加载，无法生成图片');
